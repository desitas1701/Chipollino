...
class Interpreter {
  ...
  private:
	...
	struct CompareTime {
		// Аргументы:
		// Выражение 1 -- Expression
		Expression expr1;
		// Выражение 2 -- Expression
		Expression expr2;
		// Максимальная размерность объектов (длина регулярного выражения ИЛИ
		// число состояний у автомата) -- Натуральное число
		int obj_max_size;
		// Число испытаний на объектах каждой размерности -- Натуральное число
		int tests_number = 20;
		// Шаг изменения размерности объектов -- Натуральное число
		int obj_size_step = 1;
	};
